  ...
  return (
    <div className="rounded-2xl bg-bg-surface p-5">
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            {quote.sector && (
              <span className="rounded-full bg-info/10 px-2.5 py-0.5 text-[10px] font-semibold text-info">
                {quote.sector}
              </span>
            )}
          </div>
      ...
      <div className="mt-3 flex gap-4 text-xs text-text-secondary">
        <span>Mkt Cap <strong className="text-text-primary font-semibold">{formatMarketCap(quote.marketCap)}</strong></span>
        {quote.pe != null && <span>P/E <strong className="text-text-primary font-semibold">{quote.pe.toFixed(1)}</strong></span>}
        {quote.beta != null && <span>Beta <strong className="text-text-primary font-semibold">{quote.beta.toFixed(2)}</strong></span>}
      </div>
      ...
      <div className="mt-3">
        ...
        <div className="relative h-1.5 rounded-full bg-border">
          <div
            className="absolute top-1/2 h-3.5 w-3.5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-info ring-2 ring-white shadow-sm"
